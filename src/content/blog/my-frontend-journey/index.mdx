...
For CHEESE Search, I initially tried embedding Plotly charts directly into the React interface for molecular similarity visualizations. The bundle size ballooned to 8MB, and the molecular drawing interface (Ketcher) started glitching due to conflicting event handlers. The solution was to deploy our 3D molecular viewers as separate Dash applications and embed them as iframes, keeping the main React app lightweight while preserving the interactive functionality.
